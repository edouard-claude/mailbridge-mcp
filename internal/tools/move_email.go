...
func registerMoveEmail(s *server.MCPServer, cfg *config.Config, pool *imappool.Pool) {
	tool := mcp.NewTool("move_email",
		mcp.WithDescription("Move an email to a different mailbox folder (e.g. from INBOX to Archive or Trash)."),
		mcp.WithString("account_id",
			mcp.Description("Account identifier"),
			mcp.Required(),
		),
		mcp.WithNumber("uid",
			mcp.Description("UID of the email to move"),
			mcp.Required(),
		),
		...
		mcp.WithString("to_mailbox",
			mcp.Description("Destination mailbox (e.g. 'Archive', 'Trash', 'INBOX.Processed')"),
			mcp.Required(),
		),
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if err := imappool.MoveEmail(client, fromMailbox, uint32(uid), toMailbox); err != nil {
			return mcp.NewToolResultError(fmt.Sprintf("move failed: %v", err)), nil
		}

		return mcp.NewToolResultText(fmt.Sprintf("Email UID %d moved from %s to %s.", uid, fromMailbox, toMailbox)), nil
	})
}
